<?php
//*********************************加载权限文件（start here）**********************************************************************
if((int)$_SESSION['ly200_SupplierUserId']){
	...
	if((int)$_SESSION['ly200_SupplierGroupId']!=1){	//非超级管理员权限重载
		@include($site_root_path."/inc/supplier_manage/permit/{$_SESSION['ly200_SupplierUserId']}.php");
		foreach($manage_menu as $group_key=>$group){
			foreach($group as $key=>$value){
				if($menu[$key]==1){
