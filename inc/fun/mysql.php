...
class db_class{
	...
	function get_rand($table, $where=1, $field='t1.*', $id_field, $row_count=1){	//返回随机N条记录
		$this->query("select $field from $table as t1 join(select round(rand()*((select max($id_field) from $table where $where)-(select min($id_field) from $table where $where))+(select min($id_field) from $table where $where)) as $id_field) as t2 where t1.$id_field>=t2.$id_field and $where order by t1.$id_field asc limit $row_count");
		$result=array();
		while($this->next_record()){
			$result[]=$this->record;
		}
