	...
	?>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="item_list">
			...
			<tr class="item_list item_list_out" onmouseover="this.className='item_list item_list_over';" onmouseout="this.className='item_list item_list_out';" align="center">
				...
				<td nowrap><?=iconv_price(((1-$order_row[$i]['Discount'])*$order_row[$i]['TotalPrice']+$order_row[$i]['ShippingPrice'])*(1+$order_row[$i]['PayAdditionalFee']/100));?></td>
			...
			<form action="<?=$member_url.'?'.$query_string;?>" method="post" name="member_cancel_orders_form" OnSubmit="return checkForm(this);">
				<div class="cancel">
					<div class="info">
						...
						Thanks for your Order, Please tell us the reason you want to cancel:
					</div>
		...
		<div class="detail_card">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="order_info">
			  ...
			  <tr>
				...
				<td><?php if($order_row['PayAdditionalFee']!=0){?><?=iconv_price((1-$order_row['Discount'])*$order_row['TotalPrice']+$order_row['ShippingPrice']);?> * <?=$order_row['PayAdditionalFee'];?>% = <?=$order_row['PayAdditionalFee']<0?'-':'';?><?=iconv_price(((1-$order_row['Discount'])*$order_row['TotalPrice']+$order_row['ShippingPrice'])*(abs($order_row['PayAdditionalFee'])/100));?><?php }else{?><?=iconv_price(0);?><?php }?></td>
			  </tr>
			  <tr>
				<td>Grand Total:</td>
