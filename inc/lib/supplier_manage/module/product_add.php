...
$act=(int)$_GET['act'];
if($_POST){
$member_IsUse = $db->get_one("member","MemberId='$SupplierId'","IsUse");
	if($member_IsUse['IsUse'] =='1'){
		echo '<script>alert("商户未审核,请耐心等待客服审核");history.go(-1);</script>';
		//header("Refresh:0;url=account.php?module=create&act=1");
		exit;
	}elseif($member_IsUse['IsUse'] =='0'){
		echo '<script>alert("商户未通过审核,请联系客服了解更多信息");history.go(-1);</script>';
		//header("Refresh:0;url=account.php?module=create&act=1");
		exit;
	}
	...
	// }else{
	// 	echo '<script>alert("'.$website_language['ylfy'.$lang]['zsxz1ys'].'");history.go(-1);</script>';
	// 	exit;
	...
	$_SESSION['CateId_add1'] = $CateId_1;
	$_SESSION['CateId_add2'] = $CateId_2;
	$_SESSION['CateId_add3'] = $CateId_3;
	...
	get_cfg('product.description') && $Description2=save_remote_img($_POST['Description_lang_1'], $save_dir);
	get_cfg('product.description') && $Description3=save_remote_img($_POST['Description_lang_2'], $save_dir);

	$db->insert('product_description', array(
			'ProId'			=>	$ProId,
			'Description'	=>	$Description,
			'Description_lang_1'	=>	$Description2,
			'Description_lang_2'	=>	$Description3
		)
	);
	...
	$Descriptiona=save_remote_img($_POST['Description'], $save_dir);
	$Descriptiona2=save_remote_img($_POST['Description_lang_1'], $save_dir);
	$Descriptiona3=save_remote_img($_POST['Description_lang_2'], $save_dir);
	$db->update('product_description', "ProId='$ProId'", array(
			'Description'	=>	$Descriptiona,
			'Description_lang_1'	=>	$Descriptiona2,
			'Description_lang_2'	=>	$Descriptiona3
		)
	);
	//保存扩展数据
...
?>
	...
	<script charset="utf-8" src="/plugin/kindeditor/kindeditor.js"></script>
	 <?php
     	echo '<script charset="utf-8" src="/plugin/kindeditor/lang/zh'.$lang.'.js"></script>'
     ?>
    ...
    <?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
     ...
     <?php
     	echo '<link rel="stylesheet" href="/css/supplier_m/product_add'.$lang.'.css">'
     ?>
...
<div class="mask">
	...
	<div class="yx">
		...
		<div class="yhh">
			...
			<div class="y_form1">
				<span class="y_titless"><?=$website_language['supplier_m_productm'.$lang]['cpmc']?>(<?=$website_language['ylfy'.$lang]['zw']?>)：</span><div class="bitian">*</div><input class="y_title_txtbg ac" type="text" name="Name_lang_1" >
			</div>
			<div class="y_form1">
				<span class="y_titless"><?=$website_language['supplier_m_productm'.$lang]['cpmc']?>(<?=$website_language['ylfy'.$lang]['xw']?>)：</span><div class="bitian">*</div><input class="y_title_txtbg ac" type="text" name="Name" >
			</div>
			<div class="y_form1">
				<span class="y_titless"><?=$website_language['supplier_m_productm'.$lang]['cpmc']?>(<?=$website_language['ylfy'.$lang]['yw']?>)：</span><div class="bitian">*</div><input class="y_title_txtbg ac" type="text" name="Name_en" >
			</div>
			<div class="y_form1">
				...
				<select name="CateId_1" class="form_select" id="a1">
					...
					?>
						<option value="<?=$vCateId?>" <?=$vCateId == $_SESSION['CateId_add1'] ? selected :'';?>><?=$name?></option>
					<?php }?>
				</select>
				...
				<option value="0"><?=$website_language['ylfy'.$lang]['xz']?></option>
				<?php 
					if($_SESSION['CateId_add2']){
					foreach ($vCateId1 as $keymod => $valmod) {
						foreach((array)$All_Category_row['0,'.$valmod.','] as $k1 => $v1){ 
					    if($lang == '_en'){
				...
				?>
					<option value="<?=$v1CateId?>" <?=$v1CateId == $_SESSION['CateId_add2'] ? selected :'';?>><?=$name1?></option>
				<?php }}}else{?>
				...
				?>
					<option value="<?=$v2CateId?>" <?=$v2CateId == $_SESSION['CateId_add3'] ? selected :'';?>><?=$name2?></option>
				<?php }}}}else{?>
			...
			<div class="y_form">
				<span class="y_title"><?=$website_language['supplier_m_productm'.$lang]['kc']?>：</span><input class="y_title_txt" type="text" name="Stock" >
			</div>
			<div class="y_form">
				<span class="y_title"><?=$website_language['supplier_m_productm'.$lang]['qdl']?>：</span><div class="bitian">*</div><input class="y_title_txt ust ac" type="text" name="StartFrom" >
			</div>
			<div class="y_form1" style="width:100%;float:left;margin-top:10px;min-height:31px;line-height:31px;font-size:12px;color:#333;line-height: 31px;">
	        	...
	        	<div class="color_div" style="width:800px;float:left;overflow:hidden;margin-left:6px">
		        <?php 
	        		$product_color_row = $db->get_all("product_color",'1');
	        		for ($i=0; $i < count($product_color_row); $i++) { 
	        	?>
	        		<div style="min-width:100px;float:left;overflow:hidden"><a data='1' class="y_input2"><input class="input_r2" type="checkbox" value="<?=$product_color_row[$i]['CId']?>" name="Color[]" id=""><span class='input_s'><?=$product_color_row[$i]['Color'.$lang]?></span></a></div>
	        	<?php }?>
	        	</div>	        		
	       	</div>
			<div style="position:relative;overflow:hidden;height:170px;float:left;margin-top:20px;width:900px">
                    ...
                    <span style="font-size:12px;color:#666;margin-left:20px;position:absolute;top:30px;left:120px">
                        <span style="display:inline-block;width:285px">1. <?=$website_language['supplier_m_productm'.$lang]['zl']?>?</span>
                       	<a class="y_input"><input class="input_r" type="radio" value="0" name="IsPatent"  checked><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['my']?></span></a>
						<a class="y_input"><input class="input_r" type="radio" value="1" name="IsPatent" ><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['you']?></span></a><br><br>

                        <span style="display:inline-block;width:285px">2. <?=$website_language['supplier_m_productm'.$lang]['zlzs']?>? </span>
                        <a class="y_input"><input class="input_r" type="radio" value="0" name="IsPatentCertificate" checked><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['my']?></span></a>
						<a class="y_input"><input class="input_r" type="radio" value="1" name="IsPatentCertificate" ><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['you']?></span></a><br><br>

                        <span style="display:inline-block;width:285px">3. <?=$website_language['supplier_m_productm'.$lang]['qqcp']?>？</span>
                        <a class="y_input"><input class="input_r" type="radio" value="0" name="IsInfringement" ><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['my']?></span></a>
						<a class="y_input"><input class="input_r" type="radio" value="1" name="IsInfringement" checked><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['you']?></span></a><br><br>

						<span style="display:inline-block;width:285px">4. <?=$website_language['supplier_m_productm'.$lang]['zcsc']?>？</span>
                        <a class="y_input"><input class="input_r" type="radio" value="0" name="IsInProduction" checked><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['my']?></span></a>
						<a class="y_input"><input class="input_r" type="radio" value="1" name="IsInProduction" ><span class='input_s'><?=$website_language['supplier_m_productm'.$lang]['you']?></span></a><br><br>
                     </span>
                    <div class="clear"></div>
       		</div>   
       		 <div class="clear"></div> 
       		<div class="y_form1 aa" style="color:#666;font-size:12px">
				<span class="y_title"><?=$website_language['supplier_m_productm'.$lang]['jg']?>（<?=$website_language['supplier_m_productm'.$lang]['dj']?>）：</span><input class="y_title_txt2 usd ac" type="text" name="Price_1">USD
				<input class="y_title_txt2 rmb ac" type="text" name="Price_2" value="<?=$product_row['Price_2']?>">RMB
				<span class="y_title3"><?=$website_language['supplier_m_productm'.$lang]['zli']?>：</span><input class="y_title_txt2" type="text" name="Weight" >KG
			</div>	
	  		<div class="y_form1">
  				<span class="y_title"><?=$website_language['supplier_m_productm'.$lang]['pfzk']?>：</span>
	  			<table class="table_p" border="0" cellspacing="0" cellpadding="0" id="wholesale_price_list" class="item_data_table">
	  			
	                <tr>	               		
	                	<td class="aa"><?=$website_language['supplier_m_productm'.$lang]['sl']?>：<input type="text" class="form_input2 us1" name="Qty[]">≤<input type="text" class="form_input2 us2" name="Qty2[]"><i style="margin-left:30px"></i><?=$website_language['supplier_m_product'.$lang]['dj']?>：<input type="text" class="form_input2 usd" name="WholesalePrice[]">USD<input type="text" class="form_input2 rmb" name="WholesalePrice2[]">RMB<i style="margin-left:30px"></i><a onClick="this.blur(); add_wholesale_price_item('wholesale_price_list');" class="red"><?=$website_language['supplier_m_product'.$lang]['tj']?></a></td>
	                </tr>
	            </table>
			</div>
			 <div class="y_form1">
        ...
        ?>
        <div class="y_form1">
            <div class="y_form" style="position:relative;"><input class="<?=$i+1?>" type="button" style="width:100px;height:31px;background:#0b8fff;border-radius:3px;border:none;position:absolute;left:105px;color:#fff" value="<?=$website_language['supplier_m_productm'.$lang]['sctp']?>"><span class="y_title"><?=$i+1?>:</span><input id="<?=$ad_path_3[$i]?>" type="text" style="width:240px;height:28px;background:#fff;border-radius:3px;border:1px solid #ccc;position:absolute;left:205px;color:#000" value=""><input id="<?=$ad_path[$i]?>" onchange="preImg(this.id,'photo<?=$ad_path[$i]?>','<?=$ad_path_3[$i]?>');" class="y_title_txt a<?=$i+1?>" value="上传图片" type="file" name="PicPath_<?=$i?>" ></div>
        </div> 
        <?php }?>
        ...
        <div style="width:800px;float:left;margin-top:40px;margin-left:90px">
	        ...
	        <?php for($i=0; $i<count(get_lang('ly200.lang_array')); $i++){?>
...
<script>
	...
	newcell.classList.add("aa");
	cell= newcell.insertCell();	
	cell.innerHTML='<?=$website_language["supplier_m_productm".$lang]["sl"]?>：<input type="text" class="form_input2 us1" name="Qty[]">≤<input type="text" class="form_input2 us2" name="Qty2[]"><i style="margin-left:30px"></i><?=$website_language["supplier_m_product".$lang]["dj"]?>：<input type="text" class="form_input2 usd" name="WholesalePrice[]">USD<input type="text" class="form_input2 rmb" name="WholesalePrice2[]">RMB<i style="margin-left:30px"></i><a onclick="document.getElementById(\'wholesale_price_list\').deleteRow(this.parentNode.parentNode.rowIndex);" class="del2">&nbsp;</a>';
	usd();
	us1();
	}
