...
check_permit('gift_category', 'gift.category.mod');
...
if($_POST){
	...
	if(get_cfg('gift.category.upload_pic')){
		...
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			...
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('gift.category.pic_width'), get_cfg('gift.category.pic_height'));
			del_file($S_PicPath);
	...
	if($UnderTheCateId==0){
		...
		$UId=get_UId_by_CateId($UnderTheCateId, 'gift_category');
		$Dept=substr_count($UId, ',');
	}
	
	$SubUId=get_UId_by_CateId($CateId, 'gift_category');	//下级类别的UId
	$CurUId=$db->get_value('gift_category', "CateId='$CateId'", 'UId');
	$db->query("update gift_category set UId=replace(UId, '$CurUId', '$UId') where UId like '{$SubUId}%'");
	...
	if(get_cfg('gift.category.description')){
		$Description=save_remote_img($_POST['Description'], $save_dir);
		
		$db->update('gift_category_description', "CateId='$CateId'", array(
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('gift_category', array('Category', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('gift_category_description', 'Description');
	...
	set_page_url('gift_category', "CateId='$CateId'", get_cfg('gift.category.page_url'), 0);
	
	save_manage_log('更新礼品类别:'.$Category);
...
$category_row=$db->get_one('gift_category', "CateId='$CateId'");
$category_description_row=$db->get_one('gift_category_description', "CateId='$CateId'");

include('../../inc/manage/header.php');
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('gift.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
<div class="div_con">
    ...
    <form method="post" name="act_form" id="act_form" class="act_form" action="category_mod.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
        <div class="table">
			...
			<?php
            if(get_cfg('gift.category.dept')>1){
                $o_dept=$category_row['Dept']-($db->get_max('gift_category', "UId like '{$category_row['UId']}{$category_row['CateId']},%'", 'Dept')-get_cfg('gift.category.dept'));	//剩余可用的级数
                $e_where="CateId!='{$category_row['CateId']}' and Dept<".($category_row['SubCate']?$o_dept:get_cfg('gift.category.dept'));
            ?>
                <div class="tr">
                	<div class="form_row">
                        <font class="fl"><?=get_lang('ly200.under_the');?>:</font>
                        <span class="fl">
							<?=str_replace('<select','<select class="form_select"',ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'gift_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select')));?>
                        </span>
                        <div class="clear"></div>
                    </div>
                </div>
            ...
            <?php if(get_cfg('gift.category.upload_pic')){?>
                <div class="tr">
            ...
            <?php if(get_cfg('gift.category.seo_tkd')){?>
				<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
                    <div class="tr">
			...
			<?php if(get_cfg('gift.category.description')){?>
                <?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
