...
$page_cur='gift_brand';
check_permit('gift_brand', 'gift.brand.add');

if($_POST){
	$save_dir=get_cfg('ly200.up_file_base_dir').'gift/brand/'.date('y_m_d/', $service_time);
	$Brand=$_POST['Brand'];
	...
	if(get_cfg('gift.brand.upload_logo')){
		if($BigLogoPath=up_file($_FILES['LogoPath'], $save_dir)){
			...
			$SmallLogoPath=img_resize($BigLogoPath, '', get_cfg('gift.brand.logo_width'), get_cfg('gift.brand.logo_height'));
		}
	}
	
	$db->insert('gift_brand', array(
			'Brand'			=>	$Brand,
	...
	get_cfg('gift.brand.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('gift_brand_description', array(
			'BId'			=>	$BId,
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('gift_brand', array('Brand', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('gift_brand_description', 'Description');
		...
		for($i=1; $i<count(get_cfg('ly200.lang_array')); $i++){
			...
			if(get_cfg('gift.category.description')){
				...
				$db->update('gift_brand_description', "BId='$BId'", array(
	...
	set_page_url('gift_brand', "BId='$BId'", get_cfg('gift.brand.page_url'), 2);
	
	save_manage_log('添加礼品品牌:'.$Brand);
...
<div class="div_con">
    <form method="post" name="act_form" id="act_form" class="act_form" action="brand_add.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
    	<div class="table">
            ...
            <div class="tr">
            	<div class="tr_title"><?=get_lang('ly200.add').get_lang('gift.brand');?></div>
                <?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
                    <div class="form_row">
                        <font class="fl"><?=get_lang('gift.brand').lang_name($i, 0);?>:</font>
                        <span class="fl">
            ...
            <?php if(get_cfg('gift.brand.upload_logo')){?>
                <div class="tr">
            ...
            <?php if(get_cfg('gift.brand.seo_tkd')){?>
				<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
            ...
            <?php if(get_cfg('gift.brand.description')){?>
            	<div class="tr last">
