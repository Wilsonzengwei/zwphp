...
check_permit('gift_category', 'gift.category.add');
...
if($_POST){
	...
	get_cfg('gift.category.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('gift_category_description', array(
			'CateId'		=>	$CateId,
	...
	$db->update('manage_operation_log', 'Operation="gift_category_add"', array(
	...
	save_manage_log('添加礼品类别:'.$Category);
...
<div class="div_con">
	...
	<form method="post" name="act_form" id="act_form" class="act_form" action="category_add.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
        <div class="table">
            ...
            <div class="tr">
                ...
                <?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
                    <div class="form_row">
                        ...
                        <span class="fl">
                            <input name="Category<?=lang_name($i, 1);?>" value="" class="form_input" type="text" size="98" maxlength="40" check="<?=get_lang('ly200.filled_out').get_lang('ly200.category_name');?>!~*" <?=get_cfg('gift.category.mod_name')?'':'disabled';?>>
                        </span>
                        <div class="clear"></div>
                    </div>
                <?php } ?>  
			...
			<?php if(get_cfg('gift.category.dept')>1){ ?>
                <div class="tr">
                	<div class="form_row">
                        <font class="fl"><?=get_lang('ly200.under_the');?>:</font>
                        <span class="fl">
							<?=str_replace('<select','<select class="form_select"',ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="gift_category_add"', 'Value'), 'gift_category', 'UId="0,"', 'Dept<'.get_cfg('gift.category.dept'), get_lang('ly200.select')));?>
                        </span>
                        <div class="clear"></div>
                    </div>
                </div>
