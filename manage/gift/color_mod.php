...
$page_cur='gift_color';
check_permit('gift_color', 'gift.color.mod');
...
$CId=(int)$_GET['CId'];
$color_row=$db->get_one('gift_color', "CId='$CId'");
...
<div class="div_con">
    ...
    <form method="post" name="act_form" id="act_form" class="act_form" action="color_mod.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
    	<div class="table">
            ...
            <div class="tr last">
            	<div class="tr_title"><?=get_lang('ly200.mod').get_lang('gift.color');?></div>
				<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
                    <div class="form_row">
                        <font class="fl"><?=get_lang('gift.color').lang_name($i, 0);?>:</font>
                        <span class="fl">
                            <input name="Color<?=lang_name($i, 1);?>" value="<?=htmlspecialchars($color_row['Color'.lang_name($i, 1)]);?>" class="form_input" type="text" size="30" maxlength="40" check="<?=get_lang('ly200.filled_out').get_lang('gift.color');?>!~*">
                        </span>
                        <div class="clear"></div>
                    </div>
                <?php }?>
