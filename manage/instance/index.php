...
<div class="div_con">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="bat_form">
        <?php /*?><?php if(count($category)>1 && (get_cfg('instance.move') || get_cfg('instance.del'))){?> //以后可能需要的
        <tr>
        ...
        <tr>
            <td height="58" class="flh_150">
                <form method="get" name="instance_search_form" action="index.php" onsubmit="this.submit.disabled=true;">
                    <span class="fc_gray mgl14"><?=get_lang('ly200.name');?>:<input name="Name" type="text" size="25" maxlength='100' class="form_input"></span>
                    <?php if(count(get_cfg('ly200.lang_array'))>1 && !get_cfg('instance.add_mode')){?><span class="fc_gray mgl14"><?=get_lang('ly200.language');?>:<?=output_language_select(-1, 1, get_lang('ly200.select'));?></span><?php }?>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
        ...
        <tr align="center">
            ...
            <?php if(get_cfg('instance.mod')){?><td nowrap><a href="mod.php?<?=$all_query_string;?>&CaseId=<?=$instance_row[$i]['CaseId']?>" class="mod"></a></td><?php }?>
        </tr>
        <?php }?>
        ...
        <tr>
            <td colspan="20" class="bottom_act">
            	<div class="fl mgl20">
					<?php if(get_cfg('instance.order')){?><input name="instance_order" class="list_button transition" id="instance_order" type="button" onClick="click_button(this, 'list_form', 'list_form_action')" value="<?=get_lang('ly200.order');?>"><?php }?>
                    <?php if((count($category)>1 && get_cfg('instance.move')) || get_cfg('instance.del')){?><input name="button" class="list_button transition" type="button" onClick='change_all("select_CaseId[]");' value="<?=get_lang('ly200.anti_select');?>"><?php }?>
                    <?php /*?><?php if(count($category)>1 && get_cfg('instance.move')){?>
                        <?=get_lang('ly200.move_selected_to');?>:<?=$select_category;?>
                        <input name="instance_move" id="instance_move" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'list_form_action')" value="<?=get_lang('ly200.move');?>">
                    <?php }?><?php */?>
                    <?php if(get_cfg('instance.del')){?><input name="instance_del" id="instance_del" type="button" class="list_button transition" onClick="if(!confirm('<?=get_lang('ly200.confirm_del');?>')){return false;}else{click_button(this, 'list_form', 'list_form_action');};" value="<?=get_lang('ly200.del');?>"><?php }?>
