	<div class="header" style="height:42px;">
		<div class="float_left">
        	<span><?=get_lang('menu.service_status');?></span>
            <?php
				$openmenu=0;
				foreach((array)$manage_menu['service_status'] as $key => $value){//开启子菜单的个数
					if(!$menu["$key"]) continue;
					$openmenu++;	
				}
				$i=0;
				foreach((array)$manage_menu['service_status'] as $key => $value){ //输出子菜单
					if(!$menu["$key"]) continue;
					$i++;
        ...
        <div class="float_right">
            ...
            <?php //if($page){ ?>
                ...
                <div class="topbutton fr">
                    <?php //if(get_cfg('service_status.add')){?><a href="add.php"  class="add" title="<?=get_lang('ly200.add');?>"></a><?php //}?>
                </div>
            <?php //} ?>
            <div class="clear"></div>
        </div>
