...
<div class="div_con">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="bat_form">
        <?php /*?><?php if(count($category)>1 && (get_cfg('download.move') || get_cfg('download.del'))){?> //以后可能需要
        <tr>
        ...
        <tr>
            <td height="58" class="flh_150">
                <form method="get" name="download_search_form" action="index.php" onsubmit="this.submit.disabled=true;">
                    ...
                    <?php if(count($category)>1){?><span class="fc_gray mgl14"><?=get_lang('ly200.category');?>:<?=$select_category;?></span><?php }?>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
        ...
        <tr align="center">
            ...
            <td nowrap><?=str_replace('<a','<a class="download"',creat_download_link($download_row[$i], ''));?></td>
            ...
            <?php if(get_cfg('download.mod')){?><td nowrap><a href="mod.php?<?=$all_query_string;?>&DId=<?=$download_row[$i]['DId']?>" title="<?=get_lang('ly200.mod');?>" class="mod"></a></td><?php }?>
        </tr>
        <?php }?>
        ...
        <tr>
            <td colspan="20" class="bottom_act">
            	<div class="fl mgl20">
					<?php if(get_cfg('download.order')){?><input name="download_order" id="download_order" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'list_form_action')" value="<?=get_lang('ly200.order');?>"><?php }?>
                    <?php if((count($category)>1 && get_cfg('download.move')) || get_cfg('download.del')){?><input name="button" type="button" class="list_button transition" onClick='change_all("select_DId[]");' value="<?=get_lang('ly200.anti_select');?>"><?php }?>
                    <?php /*?><?php if(count($category)>1 && get_cfg('download.move')){?>
                        <?=get_lang('ly200.move_selected_to');?>:<?=$select_category;?>
                        <input name="download_move" id="download_move" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'list_form_action')" value="<?=get_lang('ly200.move');?>">
                    <?php }?><?php */?>
                    <?php if(get_cfg('download.del')){?><input name="download_del" id="download_del" type="button" class="list_button transition" onClick="if(!confirm('<?=get_lang('ly200.confirm_del');?>')){return false;}else{click_button(this, 'list_form', 'list_form_action');};" value="<?=get_lang('ly200.del');?>"><?php }?>
