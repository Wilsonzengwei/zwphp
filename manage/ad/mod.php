...
if($_GET['action']=='delimg'){
    ...
    header('Location: mod.php?AId='.$AId);
	exit;
}
...
?>
    ...
    <form style="background:#fff;min-height:768px;width:95%" method="post"  action="mod.php" enctype="multipart/form-data" >
        ...
        <div class="y_form"><span class="y_title">广告名称:</span><input class="y_title_txt" name="AdPosition" value="<?=$ad_row['AdPosition'];?>" type="text"></div>
        <div class="y_form"><span class="y_title"><?=get_lang('ad.ad_position');?>:</span>
            <select name="PageName" id="">
                <option value="<?=$ad_row['PageName'];?>"><?=$ad_row['PageName'];?></option>
            </select>
        </div>
        ...
        <div style="width:800px;float:left;margin-top:40px;margin-left:90px">
            ...
            <div style="width:100px;height:70px;float:left;padding:10px;color:#666;font-size:12px">
                ...
                <div style="height:20px;line-height:20px;margin-top:10px" >
                    ...
                    <a id="img_list_<?=$i;?>_a" style="float:left" class="yxx_del" href="mod.php?action=delimg&AId=<?=$AId;?>&Field=PicPath_<?=$i;?>&PicPath=<?=$ad_row['PicPath_'.$i];?>">删除</a>
