...
if($_POST['data']=='mod_password'){
	...
	js_location("supplier_view.php?MemberId=$MemberId");
}
...
<div class="div_con">
    ...
    <div class="member_con" style="height: 100%;">
        <div class="act_menu">
            <div class="card_list">
                <div class="<?=$detail_card==0?'cur':'';?>"><a href="supplier_view.php?<?=$query_string;?>&detail_card=0" class="transition"><?=get_lang('member.base_info');?></a></div>
                
                <?php if($menu['orders']){?><div class="<?=$detail_card==4?'cur':'';?>"><a href="supplier_view.php?<?=$query_string;?>&detail_card=4" class="transition"><?=get_lang('member.order_info');?></a></div><?php }?>
                <div class="<?=$detail_card==5?'cur':'';?>"><a href="supplier_view.php?<?=$query_string;?>&detail_card=5" class="transition"><?=get_lang('member.login_info');?></a></div>
                <?php if(get_cfg('member.mod')){?><div class="<?=$detail_card==6?'cur':'';?>"><a href="supplier_view.php?<?=$query_string;?>&detail_card=6" class="transition"><?=get_lang('member.mod_password');?></a></div><?php }?>
            </div>
        </div>
		<?php if($detail_card==0){?>
            <iframe src="/manage/set/supplier.php?SupplierId=<?=$MemberId; ?>" frameborder="0" style="border:none;width:98%;margin:0 auto;height:1100px;"></iframe>
        <?php }elseif($detail_card==1){?>
            <form method="post" name="act_form" id="act_form" class="act_form" action="supplier_view.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
                    ...
                    <tr>
                        <td colspan="5" class="bottom_act">
                            ...
                            <div class="turn_page_form fr"><?=turn_bottom_page($page, $total_pages, "supplier_view.php?total_pages=$total_pages&detail_card=$detail_card&MemberId=$MemberId&page=", $row_count, '〈', '〉');?></div>
                            <div class="clear"></div>  
                        </td>
                    </tr>
                </table>
            </form>
            <?php /*?><form method="get" class="turn_page_form" action="supplier_view.php" onsubmit="javascript:turn_page(this);">
                <?=turn_page($page, $total_pages, "supplier_view.php?$query_string&detail_card=$detail_card&page=", $row_count, get_lang('ly200.pre_page'), get_lang('ly200.next_page'));?>
            ...
            <form name="list_form" id="list_form" class="list_form" method="post" action="index.php"> 
                <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
                    ...
                    <tr align="center">
                        ...
                        <td><a href="javascript:void(0);" onclick="this.blur(); parent.openWindows('win_orders', '<?=get_lang('orders.orders_manage');?>', 'orders/supplier_view.php?OrderId=<?=$list_row[$i]['OrderId'];?>');"><img src="../images/view.gif" alt="<?=get_lang('ly200.view');?>" /></a></td>
                    </tr>
                    <?php }?>
                    <tr>
        ...
        <?php }elseif($detail_card==6 && get_cfg('member.mod')){?>
            <form method="post" name="act_form" id="act_form" class="act_form" action="supplier_view.php" onsubmit="return checkForm(this);">
            	<div class="table">
                	<div class="tr">
