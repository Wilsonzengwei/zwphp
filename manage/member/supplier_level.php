...
$page_cur = 'member_level_manage';
check_permit('member_level_manage');
if($_POST['list_form_action']=='level_del'){
    ...
    save_manage_log('批量删除商户等级');
    
    $page=(int)$_POST['page'];
    $query_string=urldecode($_POST['query_string']);

    echo '<script>alert("批量删除成功");</script>';
    header('Refresh:0;url=supplier_level.php?$query_string&page=$page"');
    exit;
    }else{
...
?>
<form style="width:100%;background:#fff;height:60px;line-height:60px;" method="get" action="supplier_level.php">
        ...
        <span style="font-size:14px;color:#666">等级:</span>
            <select style="width:100px;height:25px;border-radius:3px;font-size:12px;color:#666" name="Level_s" id="">
                ...
                <?php 
                    $level_row_r = $db->get_all('member_level',"IsSupplier=1",'*',"UpgradePriceS asc");
...
<form style="width:100%;background:#ececec;margin-top:30px" class="list_form" name="list_form" id="list_form" method="post" action="level.php">
    <div>
        <a href="supplier_level_add.php" style="display:inline-block;margin:5px;height:25px;line-height:25px;padding:0px 20px;color:#fff;font-size:12px;background:#0b8fff;border:none;border-radius:3px">添加等级</a>          
            ...
            <table class="haha" width="100%" border="0" cellpadding="0" cellspacing="1">
                ...
                <tr class="list" style="height:60px;width:100%;" align="center">                   
                    ...
                    <td class="operation" style="border-bottom:1px solid #e5e5e5;border-right:1px solid #e5e5e5" width="10%" >
                        <a href="supplier_level_mod.php?LId=<?=$level_row[$i]['LId']?>">修改</a>                      
                        <a href="supplier_level_mod.php?LId=<?=$level_row[$i]['LId']?>&act=del_level_r">删除</a>
                        <a href="">查看</a>
                    </td>                  
                </tr>           
               <?php }?>
