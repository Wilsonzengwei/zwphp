...
<div class="div_con">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
        ...
        <tr align="center">
            ...
            <?php if(get_cfg('product.brand.order')){?><td><input type="text" name="MyOrder[]" class="form_input" onkeyup="set_number(this, 0);" onpaste="set_number(this, 0);" value="<?=$brand_row[$i]['MyOrder'];?>" size="5" maxlength="10"><input name="BId[]" type="hidden" value="<?=$brand_row[$i]['BId'];?>"></td><?php }?>
            ...
            <?php if(get_cfg('product.brand.mod')){?><td nowrap><a href="brand_mod.php?BId=<?=$brand_row[$i]['BId']?>" title="<?=get_lang('ly200.mod');?>" class="mod"></a></td><?php }?>
        </tr>
        <?php }?>
        ...
        <tr>
            <td colspan="20" class="bottom_act">
            	<div class="fl mgl20">
					<?php if(get_cfg('product.brand.order')){?><input name="brand_order" id="brand_order" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'list_form_action')" value="<?=get_lang('ly200.order');?>"><?php }?>
