...
<div class="div_con">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
        ...
        <tr align="center">
            ...
            <?php if(get_cfg('product.color.order')){?><td><input name="MyOrder[]" class="form_input" type="text" size="5" maxlength="10" onkeyup="set_number(this, 0);" onpaste="set_number(this, 0);" value="<?=htmlspecialchars($color_row[$i]['MyOrder']);?>" /><input type="hidden" name="CId[]" value="<?=$color_row[$i]['CId'];?>" /></td><?php }?>
            ...
            <?php if(get_cfg('product.color.mod')){?><td nowrap><a href="color_mod.php?CId=<?=$color_row[$i]['CId'];?>" title="<?=get_lang('ly200.mod');?>" class="mod"></a></td><?php }?>
        </tr>
        <?php }?>
        ...
        <tr>
            <td colspan="6" class="bottom_act">
            	<div class="fl mgl20">
					<?php if(get_cfg('product.color.order')){?><input name="order_color" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'action');" value="<?=get_lang('ly200.order');?>"><?php }?>
