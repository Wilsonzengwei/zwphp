...
<div class="div_con">
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="0" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='category_list_form_title'>
        ...
        $n=$m=0;
        $ext_category=array();
        $category_row=ouput_Category_to_Array_hill('info_category');
        ...
        for($i=0; $i<count($category_row); $i++){
			...
			$cate_row_count = (int)$db->get_row_count('product_category',get_search_where_by_CateId($category_row[$i]['CateId'],'info_category')." and CateId!='{$category_row[$i]['CateId']}'");
        ?>
        <tr align="center" class="category" id="category_<?=$m++;?>" style="display:<?=$category_row[$i]['Dept']==1?'':'none';?>;">
            ...
            <td nowrap align="left" class="category_list_form_catename">
                ...
                <div><a href="<?=get_url('info_category', $category_row[$i]);?>" target="_blank"><?=list_all_lang_data($category_row[$i], 'Category');?><?=$cate_row_count?" ($cate_row_count) ":''; ?></a></div>
                <?php if(get_cfg('info.category.upload_pic')){?><div><?=creat_imgLink_by_sImg($category_row[$i]['PicPath'], 20, 20);?></div><?php }?>
            </td>
            <?php if(get_cfg('info.category.mod')){?><td><a href="category_mod.php?CateId=<?=$category_row[$i]['CateId'];?>" title="<?=get_lang('ly200.mod');?>" class="mod"></a></td><?php }?>
        </tr>
        <?php }?>
        ...
        <tr>
            <td colspan="4" class="bottom_act">
            	<div class="fl mgl20">
					<?php if(get_cfg('info.category.order')){?><input name="order_category" type="button" class="list_button transition" onClick="click_button(this, 'category_list_form', 'action');" value="<?=get_lang('ly200.order');?>"><?php }?>
