...
$page_cur = 'info_two';
check_permit('info_two', 'info_two.add');
...
if($_POST){
	...
	$db->insert('info_two_contents', array(
			'InfoId'	=>	$InfoId,
			'Contents'	=>	$Contents
		)
	);
	...
	$db->update('manage_operation_log', 'Operation="info_two_add"', array(
			'Value'	=>	$CateId
		)
	);
    ...
    <form method="post" name="act_form" id="act_form" class="act_form" action="add.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
    	...
    	<div class="table" id="mouse_trBgcolor_table">
        	<div class="table_bg"></div>
           	<div class="tr">
            	<div class="tr_title"><?=get_lang('ly200.add').get_lang('info_two.modelname');?>:</div>
                <div class="form_row">
                ...
                <?php if($db->get_row_count('info_two_category')>1){?>
                    <div class="form_row">
                        ...
                        <span class="fl">
                            <?=str_replace('<select','<select class="form_select"',ouput_Category_to_Select('CateId', $db->get_value('manage_operation_log', 'Operation="info_two_add"', 'Value'), 'info_two_category', 'UId="0,"', 1, get_lang('ly200.select')));?>
                        </span>
                        <div class="clear"></div>
                    </div>
                <?php }?>
                ...
                <div class="form_row">
                    <span class="fl">
                        <?php if(get_cfg('info_two.is_video')){?>
                            <div class="items" onClick="checkproperty(this)">视频</div>
                            <input name="IsVideo" type="hidden" value="0">
                        <?php }?>
                        <?php if(get_cfg('info_two.is_img')){?>
                            <div class="items" onClick="checkproperty(this)">图片</div>
                            <input name="IsImg" type="hidden" value="0">
                        <?php }?>
                    </span>
                    <div class="clear"></div>
                </div>
                    ...
                    <div class="form_row">
                        ...
                        <span class="fl">
                            ...
                            <?php if(get_cfg('info_two.is_hot')){?>
                                <div class="items" onClick="checkproperty(this)"><?=get_lang('info_two.is_hot');?></div>
