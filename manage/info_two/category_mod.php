...
check_permit('info_two_category', 'info_two.category.mod');
...
if($_POST){
	...
	if(get_cfg('info_two.category.upload_pic')){
		...
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			...
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('info_two.category.pic_width'), get_cfg('info_two.category.pic_height'));
			del_file($S_PicPath);
	...
	if($UnderTheCateId==0){
		...
		$UId=get_UId_by_CateId($UnderTheCateId, 'info_two_category');
		$Dept=substr_count($UId, ',');
	}
	
	$SubUId=get_UId_by_CateId($CateId, 'info_two_category');	//下级类别的UId
	$CurUId=$db->get_value('info_two_category', "CateId='$CateId'", 'UId');
	$db->query("update info_two_category set UId=replace(UId, '$CurUId', '$UId') where UId like '{$SubUId}%'");
	...
	if(get_cfg('info_two.category.description')){
		$Description=save_remote_img($_POST['Description'], $save_dir);
		
		$db->update('info_two_category_description', "CateId='$CateId'", array(
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('info_two_category', array('Category', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('info_two_category_description', 'Description');
	...
	set_page_url('info_two_category', "CateId='$CateId'", get_cfg('info_two.category.page_url'), 0);
...
$category_row=$db->get_one('info_two_category', "CateId='$CateId'");
$category_description_row=$db->get_one('info_two_category_description', "CateId='$CateId'");
...
<div class="div_con">
    ...
    <form method="post" name="act_form" id="act_form" class="act_form" action="category_mod.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
        <div class="table">
			...
			<?php
            if(get_cfg('info_two.category.dept')>1){
                $o_dept=$category_row['Dept']-($db->get_max('info_two_category', "UId like '{$category_row['UId']}{$category_row['CateId']},%'", 'Dept')-get_cfg('product.category.dept'));	//剩余可用的级数
                $e_where="CateId!='{$category_row['CateId']}' and Dept<".($category_row['SubCate']?$o_dept:get_cfg('info_two.category.dept'));
            ?>
                <div class="tr">
                	<div class="form_row">
                        <font class="fl"><?=get_lang('ly200.under_the');?>:</font>
                        <span class="fl">
							<?=str_replace('<select','<select class="form_select"',ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'info_two_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select')));?>
                        </span>
                        <div class="clear"></div>
                    </div>
                </div>
            ...
            <?php if(get_cfg('info_two.category.upload_pic')){?>
                <div class="tr">
            ...
            <?php if(get_cfg('info_two.category.seo_tkd')){?>
				<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			...
			<?php if(get_cfg('info_two.category.description')){?>
                <?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
                    <div class="tr<?=$i==count(get_cfg('ly200.lang_array'))-1?' last':'';?>">
