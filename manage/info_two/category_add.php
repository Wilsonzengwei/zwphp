...
check_permit('info_two_category', 'info_two.category.add');
...
if($_POST){
	...
	$db->insert('info_two_category', array(
			'UId'			=>	$UId,
			'Category'		=>	$Category,
	...
	get_cfg('info_two.category.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('info_two_category_description', array(
			'CateId'		=>	$CateId,
	...
	$db->update('manage_operation_log', 'Operation="info_two_category_add"', array(
...
<div class="div_con">
    ...
    <form method="post" name="act_form" id="act_form" class="act_form" action="category_add.php" enctype="multipart/form-data" onsubmit="return checkForm(this);">
        <div class="table">
            ...
            <div class="tr">
                <div class="tr_title"><?=get_lang('ly200.add').get_lang('ly200.category');?></div>
			...
			<?php if(get_cfg('info_two.category.dept')>1){ ?>
                <div class="tr">
                	<div class="form_row">
                        <font class="fl"><?=get_lang('ly200.under_the');?>:</font>
                        <span class="fl">
							<?=str_replace('<select','<select class="form_select"',ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="info_two_category_add"', 'Value'), 'info_two_category', 'UId="0,"', 'Dept<'.get_cfg('info_two.category.dept'), get_lang('ly200.select')));?>
                        </span>
                        <div class="clear"></div>
                    </div>
                </div>
