...
if($_POST){
	...
	$db->update('userinfo2',"AUId='$UserId'" ,array(
			'AUersName'		=>	$Name,
			'ALocked'		=>	$JType,
	...
	//------------------------------------------------------------------------权限(start here)------------------------------------------------------------------------
		...
		//del_file('/inc/manage/character/',"$UserId".'.php', $php_contents);
		$php_contents="<?php\r\n//管理员{$UserName}权限配置\r\n".$php_contents_menu."\r\n".$php_contents_config.'?>';
		write_file('/inc/manage/character/',"$UserId".'.php', $php_contents);
...
$userinfo2_row =$db->get_one('userinfo2',"AUId='$UserId'");
include('../../inc/manage/header.php');
?>


<form style="background:#fff;min-height:600px;width:95%" action="mod.php" method="post">
	<div style="color:#3894e1;font-size:14px;font-weight:bold;text-indent:10px;margin-top:20px;line-height:50px">添加角色</div>
	<div style="margin-top:20px"><span class="y_title">角色名称：</span><input class="y_title_txt" type="text" name="Name" value="<?=$userinfo2_row['AUersName']?>">
	</div>
	<div style="position:relative;margin-top:20px;min-height:200px"><span class="y_title">拥有权限：</span>
		<span>
			<?php 
				@include("../../inc/manage/character/status.php");
				@include("../../inc/manage/character/$UserId.php");
			...
			<div data='1' class="hhe" style="width:80%;vertical-align:middle;color:#333;font-size:14px;padding:2px 0;position:relative;left:110px;top:-20px"><img style="position:absolute;top:5px" class="hheimg" src="../images/ico/Add01.png" alt=""><span style="margin-left:20px"><?=get_lang('menu.'.$group_key);?></span>
				...
				<?php 
	                ...
	                //var_dump($permit_menu);exit;
	                
				?>
					<div style='width:100px;height:20px;float:left;padding:6px 0;color:#666;overflow:hidden'>
						<input type="checkbox" <?=$checked;?> name="<?='permit_'.$key?>" value='1'><span style="font-size:12px;margin-left:5px;"><?=get_lang($value[1])?></span>
					</div>
	...
	<div style="margin-top:20px;position:relative">
		<span style="position:relative;top:-55px" class="y_title">描述</span><input style="width:750px;height:120px;border:1px solid #ccc;border-radius:3px;margin-left:10px" type="text" name="Description" value="<?=$userinfo2_row['Description']?>">
	</div>	
	</div>	
	<div class="y_submit">
		<input type="hidden" name="AUId" value="<?=$UserId?>">
