...
<div class="list_form">
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td height="58" class="flh_150" style="height:58px;">
				<form method="get" name="product_search_form" action="view.php" onsubmit="this.submit.disabled=true;">
					...
					<?php if(get_cfg('product.item_number')){?><span class="fc_gray mgl14"><?=get_lang('product.item_number');?>:<input name="ItemNumber" class="form_input" type="text" size="15" maxlength='50'></span><?php }?>
					<?php if(get_cfg('product.model')){?><span class="fc_gray mgl14"><?=get_lang('product.model');?>:<input name="Model" class="form_input" type="text" size="15" maxlength='50'></span><?php }?>
					...
					<?php }?>
					<?=get_lang('ly200.order');?>:<select name="OrderBy" class="form_select">
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	...
	<tr align="center">
		...
		<td width="8%" nowrap align="left">
			<?=get_lang('ly200.qty');?>: <input type="text" size="5" maxlength="5" value="1" name="Qty_<?=$product_row[$i]['ProId'];?>" class="form_input" onkeyup="set_number(this, 0);" onpaste="set_number(this, 0);" /><br /><br />
			<?php if(get_cfg('product.color_ele')==1 && get_cfg('product.color_ele_mode')==1 && $product_row[$i]['ColorId']!='||'){?>
				<?=get_lang('product.color');?>: <select name="Color_<?=$product_row[$i]['ProId'];?>" class="form_select"><?php
			...
			<?php if(get_cfg('product.size_ele')==1 && get_cfg('product.size_ele_mode')==1 && $product_row[$i]['SizeId']!='||'){?>
				<?=get_lang('product.size');?>: <select name="Size_<?=$product_row[$i]['ProId'];?>" class="form_select"><?php
	...
	<tr>
		<td colspan="20" class="bottom_act">
        	<div class="fl mgl20">
                <input name="button" type="button" class="list_button transition" onClick='change_all("select_ProId[]");' value="<?=get_lang('ly200.anti_select');?>">
                <input name="add_product" id="add_product" type="button" class="list_button transition" onClick="click_button(this, 'list_form', 'act');" value="<?=get_lang('ly200.add');?>">
            ...
            <div class="turn_page_form fr"><?=turn_bottom_page($page, $total_pages, "view.php?$query_string&total_pages=$total_pages&Name=$Name&ItemNumber=$ItemNumber&Model=$Model&CateId=$CateId&ext_where=$ext_where&OrderId=$OrderId&module=$module&page=", $row_count, '〈', '〉');?></div>
            <div class="clear"></div>  
		</td>
	</tr>
	<?php }?>
