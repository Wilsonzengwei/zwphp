...
if($_POST['list_form_action']=='orders_del'){
    ...
    if(count($_POST['select_OrderId'])){
        ...
        $db->update('orders', "OrderId in($OrderId)", array(
            'Del'   =>  '0',
            ));
...
//var_dump($OrderTimeS.'and'.$OrderTimeE);exit;
...
<form style="width:100%;background:#fff;height:60px;line-height:60px;" method="get" action="index.php">
    ...
    <select style="width:130px;height:25px;border-radius:3px;font-size:12px;color:#666" name="OrderStatus" id="">
      <option value="0" >全部</option>
        <?php foreach($order_status_ary3 as $key=>$value){?>
            <?php 
                if($key == '2'){
            ...
            <table class="haha" width="100%" border="0" cellpadding="0" cellspacing="1">
                ...
                <?php 
                    ...
                    for ($i=0; $i < count($order_row); $i++) { 
                        ...
                        $mobliel = $db->get_one('member',"MemberId='$MId'","FirstName,RegIp,Supplier_Mobile");

                        $ip_area=ip_to_area($mobliel['RegIp']);
                        if($ip_area){
                        ...
                        if($moblie){*/
                        $moblie =  $mobliel['Supplier_Mobile'];
                        /*}else{
                ...
                <tr class="list" style="width:100%;height:60px;bgcolor:#fff" align="center">                   
                    ...
                    <td style="border-bottom:1px solid #e5e5e5;border-right:1px solid #e5e5e5" width="7%" ><a style="display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;width:80px;text-align:left;overflow:hidden;text-align:center"><?=$mobliel['FirstName']?></a></td>
                    ...
                    <td style="border-bottom:1px solid #e5e5e5;border-right:1px solid #e5e5e5" width="7%" ><?=$order_status_ary3[$order_row[$i]['OrderStatus']];?></td>
                    ...
                    <td class="operation" style="border-bottom:1px solid #e5e5e5;border-right:1px solid #e5e5e5" width="10%" >
                        <a href="view.php?OrderId=<?=$order_row[$i]['OrderId']?>&OId=<?=$order_row[$i]['OId']?>">查看</a>
                        <?php 
                            if($order_row[$i]['FollowStatus'] == '1'){    
                        ?>
                            <a href="#" style="display: none">跟进</a>   
                        <?php }else{?>     
                            <a href="index.php?act=FollowStatus&OrderId=<?=$order_row[$i]['OrderId']?>">跟进</a> 
                        <?php }?>                
                    </td>               
                </tr> 
                <?php }?>             
