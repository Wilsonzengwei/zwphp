...
<div id="page_contents">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		<td style="border-bottom:1px solid #ccc;" height="30"><input type="checkbox" name="MailList[]" value="<?=htmlspecialchars($member_row[$i]['Email'].'/'.$member_row[$i]['FirstName'].' '.$member_row[$i]['LastName']);?>" style="margin-top:1px;"/> <a href="javascript:void(0);" onclick="this.blur(); add_email_address(this.innerHTML);"><?=htmlspecialchars($member_row[$i]['Email'].'/'.$member_row[$i]['FirstName'].' '.$member_row[$i]['LastName']);?></a></td>
	  </tr>
	  <?php }?>
	</table>
	<form method="get" class="turn_page_form" action="mail_list.php" onsubmit="javascript:turn_page(this);" target="mail_list_iframe" style="margin-top:10px;">
		<?=str_replace('<a', '<a target="mail_list_iframe"', turn_bottom_page($page, $total_pages, 'mail_list.php?page=', $row_count, '〈', '〉'));?>
