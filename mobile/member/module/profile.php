...
if($_POST['data']=='member_profile'){
	...
	js_location("$mobile_url$member_url?profile_success=1&$query_string");
}
?>
<div id="lib_member_profile">
	...
	<?php if($_GET['profile_success']==1){?>
		...
		<div class="change_success lib_member_item_card">
			...
			<a href="<?=$mobile_url.$member_url;?>?module=index"><strong><?=$website_language['member'.$lang]['profile']['return']; ?></strong></a><br /><br />
		</div>
	<?php }else{?>
		<div class="lib_member_info"><?=$website_language['member'.$lang]['profile']['title2']; ?></div>
		<div class="form lib_member_item_card">
			<form action="<?=$mobile_url.$member_url.'?'.$query_string;?>" method="post" name="member_profile_form" OnSubmit="return checkForm(this);">
				...
				<div class="rows">
					...
					<span><input name="Supplier_Name" value="<?=$member_row['Supplier_Name']; ?>" type="text" class="form_input" check="<?=$website_language['member'.$lang]['other']['name_tips']; ?>!~*" size="40" maxlength="20"></span>
					<div class="clear"></div>
				</div>
				<div class="rows">
